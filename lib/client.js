...
Client.prototype.cancel = function(query, callback) {
  ...
  function onresponse(err, response, data) {
    if (err) {
      ...
      callback(new DruidError(util.format('Unknown error (status: %d) while cancelling query: %j', response.statusCode, data)))
    }
    else {
      callback(null)
    }
...
Client.prototype.exec = function(query, callback) {
  ...
  function onresponse(err, response, data) {
    if (err) {
      ...
      callback(new DruidError(util.format('Unknown error (status: %d) while executing query: %j', response.statusCode, data)))
    }
    else {
      callback(null, data)
    }
